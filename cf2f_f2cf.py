...
modification le 01/05/2021 Emmanuel SAID
pyinstaller --onefile cf2f_f2cf.py  -> cf2f_f2cf.exe
...
if __name__ == '__main__':
    ...
    name = sys.argv[1]
    extension_file1 = sys.argv[2].split('.')
    extension_file2 = sys.argv[3].split('.')
    #print(extension_file1, extension_file2)

    if len(sys.argv) == 4:
        if name == 'cf2f' and extension_file1[1] == 'cf' and extension_file2[1] == 'f':
            print(" colorforth  to  forth ", sys.argv[2:])
            cf2f(sys.argv[2], sys.argv[3])

        if name == 'f2cf' and extension_file1[1] == 'f' and extension_file2[1] == 'cf':
            print(" forth to   colorforth  ", sys.argv[2:])

            f2cf(sys.argv[2], sys.argv[3])
    else:
        print("pas le bon nombre de parametres  ")
    # debug('trying: %s with args %s' % (name, sys.argv[1:]))
    # debug(eval(name)(*sys.argv[1:]))

    # cf2f("okadback.cf", "okad.f")
    #f2cf("okad.f" , "okadback2.cf")
    # cf2f("okadback2.cf", "okad2.f")
