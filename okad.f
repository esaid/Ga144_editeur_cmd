...
{block 18}
:var ns [0x29b] :var nblk [0x5a0] :var nc [0x13] ( 144a12 03b 42c/22a) | , ( copyright) # 2009 ( greenarrays,inc) | , 20 load 22 load 24 load ( colors) 28 load ]
: abuf [ 10000 ] ; | *
: cbuf [ 29000 ] ; | , ( decompress) mark 30 load restore empt ]
: env [ 34 ] winver 2* + ; env ( ironment) load | , 44 46 thru 62 70 thru 88 load 84 load | , | ,
: floppy [ 52 ] ; | * | .
: dump [ 48 ] load ; | *
: flush save ;
: icons | . [ 54 ] ; | *
...
: 2*d $c3a50f 3, ;
: 2*c $e0d3 2, ; forth :var na [0x1e] :var nb [0x9] :var h [0x4062000] :var an :var aa [0x472f169] :var nz [0x4]
...
: serial ( -n) [ 112 ] ;
: fopen ( af-h) push push 0 32 ( exist) 3 0 # 0 [ 3 | , ] pop pop swap [ 4 ] * fcreate ; | , :var flng [0x1cb] flng 4 * dup ]
...
{block 64}
( - more) :var fmask [0xfffffff0] :var fnn [0xe78f3400] | cr
...
{block 80}
( qx) :var ?qxc [0x102f7e29] :var qb [0x258] 82 load ]
...
{block 94}
( improved stack display) :var nr [0x6] | , | ,
: -msg [ ' nul ] 'msg ! # 2 6 [ nr ] ! ;
: msg -msg pop 'msg ! ;
...
{block 114}
( - umbilical) :var sport [0x9] 1 sport ! ]
: rsh ( ni-n) 0 + if for 2/ next ; ] then drop ;
: sdat align create [ 24576 rez :var bi [0x1cb]
: -stream 0 [ bi ] ! ;
: stream ( -b) [ sdat a-b ] ;
: /str ( -b) [ bi ] @ [ stream ] + ; | br
...
: ctl! ( n) [ dh ] @ fesc drop ; :var ?reset [0x1] 1 ?reset ! ]
...
: exec [ $ae ] ;
: talk cold -ser +ser if drop reset $40 0 [ exec ] /frame $0 $40 [ 1600 ] +ram [ wos ] !frame ( good) ; ] then ( bad) -ser drop $bad dup abort ;
: send ( ac) for dup @ [ $15555 ] or -stream !18 | , | .. stream [ 3 ] toser [ 1 ] + next drop ;
: exch ( w n f n) push push dup [ $2 ] + [ -2 ] and [ vport ] @ pop /frame [ vport ] @ [ $12000 ] + !18 dup - [ $1 ] and drop if [ vport ] @ [ $10000 ] + !18 then +frame [ wos ] !frame pop inser -stream ; | , | ,
: ok [ $12 ] ;
: fet [ $15 ] ;
: stat [ $1b ] ;
: bstk [ $1d ] ;
...
{block 119}
( boot frames begin with a 3 word header;) | , | .. ] 100xx ( jump to xx when done) | , | .. ddd ( mem/port adr to store payload) | , | .. n ( transfer length) | , | .. [ n ( words of payload, none if) n ( is 0.)
...
: send ( sends a boot stream after talk.)
...
{block 122}
( - boot target adjacent) :var 'wall [0x10301f7a]
: swall ( nn-i) over over or [ 1 ] and drop if or [ 2 ] and ; ] then # 10 [ 100 ] / swap # 10 [ 100 ] / or [ 2 ] and [ 1 ] + ;
: wall ( nn-i) [ 'wall ] xqt ; ' swall 'wall ! ]
: mwall ( nn-i) [ 1000 ] mod swap [ 1000 ] mod | , over over or drop if swall ; | , ] then drop drop ( up) 3 ;
: 2chip [ ' mwall ] nop [ 'wall ] ! ; | , | ,
: do! align create ( @p+a!.@p+) [ $4ab7 , | , | .. $0 , $0 , ( !;;;) $b555 , ]
: do@ align create ( ...@p+) [ $2c9b7 , | , | .. $0 , ( a!@!p+;) $2be35 , ]
: doi align create [ $10000 , ]
: dopu align create ( @p+;) [ $5555 , $0 , ]
: dostk align create ( !p+dup-push!p+) [ $cdbe , | , | ,
...
{block 124}
( - stream components) :var 'pth [0x40bffa1]
...
{block 126}
( - umbilical plumbing) :var 'foc [0x1030062a]
...
{block 144}
( arrayforth tm and okad tools and designs) | , ( copyright 2009-2012 greenarrays, inc.) | , :var cuco [0x5] :var testb :var dh 0 dh ! | , :var first [0xffffffff] -1 first ! :var orgn 0 orgn ! ]
: compile [ 146 ] load ;
...
: assign ( a) pop swap ! ;
: ray ( i-a) pop 2/ 2/ + ;
: aray ( n) [compile] align call ray [ 4 ] * [ h ] +! ; | , | , ( ascii file names) 74 load | , ( png screen capture) 168 load | , ( qx) 80 load ( stack) 94 load ]
: -tape [ 1 ] 1 and ; ( for redacted systems) | , | , ( chip design) 190 194 thru mark empty | , ( okad resident) # 660 ( load mark empty)
...
{block 148}
( f18 software simulator) empty compile | , ( demo bootstream) 1242 load empty | , ( prelude) 1250 load ( boot descriptors) 1236 load | , ( engine) 1252 1266 thru ( opcodes) 1268 1274 thru | , ( boot loader) 1238 load | , ( view) 1276 1286 thru 1248 load 1288 1290 thru | , ( preserve variables) :var nmem :var n2mem
: nm2m [ nod2 ] @ !node [ n2mem ] @ mem ! | , | ... [ nod ] @ !node [ nmem ] @ mem ! ; :var big [0x2]
: puka ( nn-a) nn-n 2* $8000 + block ;
: code ( nn nn) puka push puka pop 64 move ; | , ( kbd) 1292 1294 thru | , ( ports/pins) 1296 1298 thru | , ( interactive) 1240 load | , | ,
...
{block 150}
( test code for chip) reclaim | , | , ( ide parts) 1372 1380 thru reclaim | , ( small tools) 234 load | , ( all nodes) 1350 load reclaim | , | , ( ats tests) 480 load reclaim | , | , ( polyforth) 360 load reclaim | , ( eforth) 1080 load ( reclaim) | , | , ( bridge) 644 load reclaim | , | , ( applic code) 200 load | , | ,
...
:var pad [0x109e4000] abuf block 4 * pad ! | cr
:var hld [0x109ebe78] pad @ hld ! 178 188 thru ]
...
: nns [ nnx nny * ] ;
: nnc ( nns) [ nns 2 * 144 + ] ;
: nn-n ( cyyxx-n) [ 10000 ] /mod [ nns ] * | , | .. | .. swap [ 100 ] /mod [ nnx ] * + + ;
: n-nn ( n-yyxx) [ nnx ] /mod [ 100 ] * + ; | , | ,
...
: nn-n ( and) n-nn ( convert yx notation to and from linear node nos. 1xxxx denotes second chip.)
...
: idle ( n-p) [ nns ] mod [ nnx ] /mod [ nny -1 + ] mod [ 1 ] min [ 4 ] + swap [ nnx -1 + ] mod [ 1 ] min 2* + port ;
...
: idle ( is idle p value for given) [ linear ( node.)
...
: kinds [ nnc aray | , | ,
: 0's ( n) for 0 , next ;
: +roms ( i-a) align ray [ nns 0's ]
: iz ( nn i) swap nn-n +roms ! ; | , 701 2 iz 705 10 iz 708 6 iz | , | ... 709 14 iz 713 14 iz 717 14 iz | , 117 14 iz 617 14 iz | , 1 2 iz 200 18 iz 300 4 iz | , ( eforth) 105 20 iz 106 20 iz ( sdram...) | , 7 20 iz 8 20 iz 9 20 iz 107 20 iz 108 20 iz ]
: @rom ( nn-n) nn-n nns mod +roms @ [ 22 ] less if drop [ 1418 ] + ; ] then drop [ 100 ] mod [ 1301 ] + ; | , | ,
...
: +roms ( array indexed by linear node number containing code number for rom load block.)
...
( sha256 reclaim) # 900 # 5 ( loads) | , ( greg n1 object) # 950 ( load) | , ( mangpo test) # 894 ( load) | , ( an012 sensortag) # 960 ( load) | , | , ( hardsim pwr verify) # 882 ( load) | , ( random) # 884 ( load)
...
: loader [ 210 ] ; | *
: tools [ 228 ] ; | , | ,
: selftest ( port) [ 708 ] load ; :var stp [0x9]
: autotest ( port) [ 712 ] load ; | , | , ( remember where to resume flash boot...) | , :var @res1 [0x268] :var @res2 [0x400]
...
: host ( and) target ( load block numbers for serial ide configured to work with eval board host or target chips using default usb ports.)
: bridge ( loaded to extend ide for) # 2 ( chips.) | br
...
: /b a ( specifies) [ b ( value)
: /io n ( specifies initial) [ io ( value)
: /p a ( specifies entry point)
...
{block 214}
( - routing) :var 'rte [0x40c1c3c]
: route ( i) 'pths + @ [ 'rte ] ! ;
: active ( -n) [ 'rte ] @ @ ;
: follower ( -n) [ 'rte ] @ [ 1 ] + @ ;
: +route ( n) [ 'rte ] +! active nn-n [ com ] ! ; | , | ,
: ?load c-boot @ [ 0 ] + drop -if ; ] then | , | ... rammer boot ; | , | ,
: deliver [ pth ] @ active hook ( pause) ?load | , io-reg @ - [ 0 ] + drop -if | , | ... io-reg @ io r! then | , a-reg @ - [ 0 ] + drop -if | , | ... a-reg @ lit' ra! then | , b-reg @ - [ 0 ] + drop -if | , | ... b-reg @ lit' rb! then | , 'stack [ 10 ] + @ dup and if dup | , | ... dup negate [ 10 ] + [ sp ] ! for @s+ lit' next | , | ... then drop | , p-reg @ call ;
...
{block 218}
( evb001 bridge 2-chip ide) empty compile | , | , serial load | , ( customize) -canon 0 fh orgn ! | , a-com sport ! a-bps bps ! !nam | , ( path) 2 fh load ( setup) 4 fh load | , | ,
: dac ( n) $155 and io r! ; | , | ,
: mwall ( nn-i) [ 1000 ] mod swap [ 1000 ] mod | , over over or drop if swall ; | , ] then drop drop ( up) 3 ; ' mwall 'wall ! | , | ,
: span !b0a setup !b0 !b2 | , | .. # 0 # 10000 ( hook) 0 -hook ; | , | , ( canon load)

{block 219}
( this load block compiles and configures the) | , ( ide for a 2-chip environment on the eval board) | , ( this block starts by loading the serial ide in its standard form.) -canon ( forgets the mapping of canonical words like) ] @ ( and) dup ( onto the f18 so that we may interact with x86 code and ram.) | , ( we then set) [ orgn ( so that the ide) ] compile ( will reload this block, and we set) [ sport ( to the usb port a for normal ide operations on host chip.) !nam ( updates pathname for that com port.) | , | ,
: talk ( resets host chip as in regular ide.)
: span ( resets target and builds port bridge) | , ( between nodes) # 300 ( on both chips. paths are set for bridged operation.) | , | , [ canon load ( restores mapping of canonical words onto the f18. comment this if you wish to use the 'r-words' for all f18 operations.)

{block 220}
( - paths)
: bri0a align create | , [ 708 608 to 607 600 to 500 300 to -1 , | ,
: bri0 align create | , [ 708 608 to 607 600 to 500 400 to | , 10400 10700 to 10701 10717 to 10617 10601 to | , 10501 10517 to 10417 10401 to 10301 10317 to | , 10217 10200 to 10100 10117 to 10017 10000 to | , -1 , ]
: bri2 align create | , [ 708 717 to 617 601 to 501 517 to 417 401 to | , 301 317 to 217 200 to 100 117 to 17 0 to -1 , | , | ,
: !b0a bri0a [ 'pths ] ! ;
: !b0 bri0 [ 'pths ] ! ; | ,
: !b2 bri2 [ 'pths 2 + ] ! ;

{block 221}
( in the bridged environment, paths are...) | , | .. bri0 ( used to reach and boot entire target) | , | .. | .. ( chip as well as 400/500/600/700-707.) | , | .. bri2 ( reaches rest of host chip not covered) | .. | .. ( by path 0.) | , | .. path 1 ( available for general debug use.) | , | , | .. bri0a ( only used to set bridge up.) | , | ,
...
: setup 0 rst !sync 1 rst | , 0 [ 300 ] hook !his !ours 0 -hook | , 2 -hook 1 -hook 0 -hook ;
...
{block 228}
( tools - for load descriptors)
: +wire ( ion) +node 0 1 [ 1802 ] /part /b /a 0 /p | , | .. -1 -1 -1 -1 -1 -1 -1 -1 -1 9 /rstack ; | , | ,
: l, ( nn) dup +node 100 /mod 1 and 2* swap 1 and + | .. 1714 + push 32 32 pop /part ( warm) $a9 /p ;
: ganglia nns for i -1 + n-nn l, next ; | , | , ( delays) 2 fh load

{block 229}
( load) ] tools ( to make the following generators) | , | .. ( available for your boot descriptors...) | , | , +wire ( generates perfect wire given input and) | , | .. ( output ports followed by node number.) | , | .. ( code is one word at zero, write by hand if) | , | .. ( you want the word to be anywhere else.) | , | , ganglia ( fills the top half of ram in all nodes) | .. ( of first chip with appropriate ganglia.)

{block 230}
( - delay line load descriptors) | ,
: +dly ( ion) +node /b /p $9822 $9d0a | , | .. over over over over over over over over | , | .. 10 /stack 0 /a ;
: ?rl ( nn d - nn i o nn) over push i + 1 and if | , | .. drop right left pop ; | , | .. ] then drop left right pop ;
: ?ud ( '') over push i 100 / + 1 and if | , | .. drop down up pop ; | , | .. ] then drop up down pop ; | , | ,
: +dx ( nn x) for 0 ?rl +dly 1 + next drop ;
: -dx ( '') for 1 ?rl +dly -1 + next drop ;
: +dy ( '') for 0 ?ud +dly 100 + next drop ;
: -dy ( '') for 1 ?ud +dly -100 + next drop ; | , | ,

{block 231}
( boot descriptor tools for delay lines.) | , | , ] +dly ( is used like) +wire ( to make a single delay) | .. ( line node given in,out ports and node no.) | , | , +dx +dy -dx -dy ( build delay lines going right,) | .. ( up, left and down respectively from the) | , | .. ( given starting node) [ nn ( for) x ( nodes. example) | , | .. ( delay) 504 13 +x right up 517 +dly ( fed thru) | , | .. ( left port of node) # 504 ( and emits thru up port) | .. ( of node 517, delay is 14*64 or) # 896 ( words.) | ,
...
{block 234}
( compile small tools) | , = $1a5 0 node 0 org reclaim ( perfect wire) | ,
: wire = $0 begin begin @ !b unext unext ( wire ;) | , | , = $1 [ 1802 bin reclaim exit

{block 235}
( this bin has small tools.) | , | , ] perfect wire ( provides maximum bandwidth with) | , | .. ( minimal latency and jitter of one unext per) | , | .. ( 262k words transferred. the one word of code) | .. ( remains in the intruction register till) | , | .. ( reset. return stack must be all -1s.)

{block 236}
( delay line templates mk2) | , | , = $1 100 node 0 org reclaim ( perfect source)
: src $20000 $30000 over over over over over over | , | .. over over begin begin !b unext unext ; | , | , = $7 101 node 0 org reclaim ( shover)
: shove ( n-n) | , | .. = $0 @ @p !b [ .. ] !b @p @ .. | , | .. = $2 !b @p !b [ .. ] !b !+ !b .. | , | .. = $4 shove ; | , | , = $5 102 node 0 org reclaim ( sucker) | ,
: suck = $0 @ ( drop) @ !b @ ( drop) suck ; | , = $2 reclaim exit | ,

{block 237}
( template for delay line bandwidth measurement) | , | , ] perfect wire ( is fast enough to guarantee no) | , | .. ( back pressure or frustration for nodes on) | , | .. ( either side at these speeds.) | , | , perfect source ( generates input for the shover,) | .. ( always ready with the next word long before) | .. ( the shover is reading.) | , | , shove ( converts a single word of data to the) | , | .. ( three word packet used within a delay line.) | , suck ( receives three words at the far end with) | , | .. ( no additional back pressure, producing one) | , | .. ( word at the output.) | , | , [ this version has optimal opcode sequence | , | .. moving one word per 52 ns regardless of the | , | .. length of delay line! much better than mk1 | , | .. which was running 250ns per word | , | .. at only 15 nodes.

{block 238}
( -- delay line example) | , ( source) 0 ( down) # 417 right 502 +wire | , | .. 100 /ram 0 /p $30000 $20000 over over | , | .. | .. over over over over over over 10 /stack | , ( shover down up) # 517 | , | .. right left 503 +wire 101 /ram | , | , ( delay line) 504 13 +dx right up 517 +dly | , | .. up right 617 +dly 616 15 -dx | , | , ( sucker) left right 601 +wire 102 /ram | , ( sink) right io 600 +wire

{block 239}
( this is an example of a delay line built for) | , | .. ( throughput measurement.) | , | , ( the source is begun as a wire to take care of) | , | .. ( the return stack initialization.) | , ( the sink drives a pin with data originating on) | .. ( the source's stack.)
...
: up [ $145 ] ; | *
: data [ $141 ] ; | *
: ldata [ $171 ] ; | , ( streams) 2 fh 4 loads | , | ,
: body ( w-w) follower [ 0 ] + drop -if | , ?load post ; ] then -load -post pump | , [ 1 ] +route body [ -1 ] +route ?load post ; | ,
: length ( -n) 1 [ 'pth ] @ [ 1 ] + begin dup @ - [ 0 ] + | , -while - nn-n [ com ] ! c-boot @ [ 0 ] + -if [ -4 ] + | , then ( load) [ 5 ] + ( pump) [ 5 ] + ( post) @post + u+ | , [ 1 ] + end then drop drop ( -1pump) [ -5 ] + ; | , | ,
: head ( a-w) active nn-n [ com ] ! [ $10000 ] + !18 | , [ 0 ] side dup !18 length dup !18 | , swap [ $12000 ] + !18 [ 1 ] +route ;
: frame exec
: fram ( a) head [ -1 ] + body drop [ -1 ] +route | , | ... c-boot @ [ 0 ] + drop -if 0 0 0 /part then | , | ... p-reg @ booter /root ; | , | , ( default path) 22 fh load
...
: frame ( make the whole boot frame) | , | ... # -1 ( for focus , +3 for header)
: fram [ a ( make a bootframe using a different) | , ( continuation address)
...
: @s+ ( 'stack) [ sp ] @ + @ 1 [ sp ] +! ;
: !s- ( n 'stack) [ sp ] @ + ! -1 [ sp ] +! ;
: /stack ( stuff count) | , | .. 10 [ sp ] ! dup 'stack !s- for 'stack !s- next ;
: /rstack ( stuff count) | , | .. 10 [ sp ] ! dup 'rstack !s- for | , | .. | .. 'rstack !s- next ;
...
: /io ( initial value of io register)
...
: /rstack ( specify return stack initialization)

{block 244}
( - framer) :var 'pth [0x40c0b72] :var 'wall [0x10302837]
: b-a ( b-a) [ 3 ] + 2/ 2/ ; | *
: a-b ( a-b) 2* 2* ;
: create ( -a) pop b-a ;
: route pop b-a | *
...
: active ( -n) [ 'pth ] @ @ ;
: follower ( -n) [ 'pth ] @ [ 1 ] + @ ;
: +route ( n) [ 'pth ] +! active nn-n [ com ] ! ; | , :var wd [0x4841634]
: 'stream ( -a) [ nnc 2 * $8000 + block 1 + ] ; | , ( clear) 'stream 1 + wd ! ]
: stream ( -ac) [ 'stream 1 + ] dup [ wd ] @ - + - | , dup [ 'stream -1 + ] ! ;
: strlen ( -n) stream nip ;
: !18 ( n) [ $15555 ] or [ wd ] @ ! 1 [ wd ] +! ; | ,
: swall ( nn-i) over over or [ 1 ] and drop if or [ 2 ] and ; ] then # 10 [ 100 ] / swap # 10 [ 100 ] / or [ 2 ] and [ 1 ] + ;
: wall ( nn-i) [ 'wall ] xqt ; ' swall 'wall ! ]
: side ( n-a) [ 'pth ] @ + dup @ swap [ 1 ] + @ wall port ;
: 0/2 ( n-0/2) [ 0 ] + drop -if [ 0 ] ; ] then [ 2 ] ;
: @post ( -n) a-reg @ 0/2 b-reg @ 0/2 + io-reg @ | , | .. 0/2 2* + 'stack [ 10 ] + @ [ 2 ] * + | , | .. 'rstack [ 10 ] + @ [ 2 ] * + [ 1 ] + ;
...
{block 246}
( - framer)
: pump ( w-w) [ -5 ] + ( @pdupa!@p) [ $4daf ] !18 | , [ 0 ] side [ $12000 ] + !18 dup [ -1 ] + !18 | , ( push!..) [ $2fab2 ] !18 ( @p!unext.) [ $5a72 ] !18 ;
: post ( /io) | .. io-reg @ - [ 0 ] + drop -if | , ( @pb!..) [ $4bb2 ] !18 io !18 | , ( @p!b..) [ $5bb2 ] !18 io-reg @ !18 then | , ( /a) | .. a-reg @ - [ 0 ] + drop -if | , ( @pa!..) [ $4ab2 ] !18 a-reg @ !18 then | , ( /b) | .. b-reg @ - [ 0 ] + drop -if | , ( @pb!..) [ $4bb2 ] !18 b-reg @ !18 then | , ( /rstack) | .. 'rstack [ 10 ] + @ dup and if dup | , | .. dup negate [ 10 ] + [ sp ] ! for ( @ppush..) [ $48b2 ] !18 | , | .. 'rstack @s+ !18 next then drop | , ( /stack) | .. 'stack [ 10 ] + @ dup and if dup | , | .. dup negate [ 10 ] + [ sp ] ! for ( @p..) [ $49b2 ] !18 | , | .. 'stack @s+ !18 next then drop | , ( /p always) p-reg @ [ $10000 ] + !18 ; | ,
: +ram ( acb) nn-n 2* [ $8000 ] + block u+ | , for dup @ [ $15555 ] or !18 [ 1 ] + next drop ;
: +load ( acb) push ( @pa!@p.) [ $4a12 ] !18 | , | .. over !18 dup [ -1 ] + !18 ( push...) [ $2e9b2 ] !18 | , ( @p!+unext.) [ $5872 ] !18 pop +ram ;
...
{block 248}
( - framer)
: ?load ( w-w) c-boot @ [ 0 ] + drop -if ; ] then | , booter +load ;
: -load ( w-w) c-boot @ [ 0 ] + -if drop ; ] then | , [ 5 ] + negate + ;
: -post ( w-w) @post negate + ; | , | ,
: /adjust begin strlen 7 and drop while | , 0 !18 end then ;
: 1adjust /adjust strlen [ @res1 ] ! ;
: 2adjust /adjust strlen [ @res2 ] ! ;
: scrub ( n) +node [ -1 ] dup dup /part | , -1 /a io /b warm /p ;
: fresh begin active scrub [ 1 ] +route | , active dup and drop -until 708 +node $aa /p ; | , | ,
: /root ( a' a c b) push push push | , $10000 +or !18 pop dup !18 pop dup !18 | , [ 0 ] + if pop +ram ; ] then drop drop pop drop ; | , | ,
: mwall ( nn-i) [ 1000 ] mod swap [ 1000 ] mod | , over over or drop if swall ; | , ] then drop drop ( up) 3 ;
: 2chip [ ' mwall ] nop [ 'wall ] ! ;
...
serial load -canon options @ load | cr
...
nores 285 list | cr
.noboot pause wait ( ** install noboot jumper) | cr
talk ( check a-com) | , 0 705 hook $0 64 eraser boot | , .boot pause wait ( *** remove noboot jumper) | , .erasing pause erase ( bootstream) chill | , | , 0 705 hook $0 64 writer boot focus | cr
0 706 hook $0 64 helper boot | br

( flash commands) flash load | , 285 list nosay | cr
.burning pause burn chill | cr
.checking pause check | cr
( spi ready) 0 705 hook 0 64 1606 boot 4 call | , overlay

{block 253}
( writing a boot stream into either flash) | , | , ( force length to) # 0 ( mod) # 8 ( so operation ends) | , ( at a) # 16 ( bit boundary) | , | ,
: 18burn ( sdn) | , [ s ( source address of buffer in host memory) | , d ( destination) # 8 ( bit address in flash) | , n ( length of stream in) # 18 ( bit words) | ,
: eraser | *
: writer | *
: helper | * ( and)
: flash ( are defined in options to choose old or new flash)

{block 254}
( - old flash bootstream burning tools)
: read ( a) left lit' ra! | , | .. split swap lit' lit' [ $23 ] call ;
: r left lit' ra! [ $29 ] call upd ;
: commence left lit' ra! [ len ] @ 8 / -1 + | , | .. [ dest ] @ split swap lit' lit' lit' [ $2d ] call ;
: git ( a-an) dup [ 1 ] + swap @ [ $15555 ] or [ $3ffff ] and ;
: get ( a-a) git lit' ;
: burning [ source ] @ [ len ] @ for get !+ next drop ;
: fetch ( -n) [ $37 ] call [ $3a ] r@ ;
: check ( -t) [ dest ] @ read [ source ] @ [ len ] @ | , | .. for git fetch over ( pause) over or | , | .. drop if pop fail ; | , | .. ] then drop drop next drop good ;
...
{block 255}
( - old flash bootstream burning tools)
: read ( initiate a read instruction at) | , | .. ( the) # 20 ( bit address)
: commence ( initiate a write instruction)
: git ( read an) # 18 ( bit word from the buffer)
: get ( and place on stack)
: burning ( read and send whole bootstream)
: fetch ( read next word from flash and) | , | .. ( bring to the pc stack)
: check ( compare flash to buffer)
: burn ( initiate write and send bootstream) | , | ,
: r | *
: f | *
: b ( were used debugging this code) | , | .. ( not part of the current system)

{block 256}
( - old flash bootstream burning helper) $20 org ]
: focus = $20 @p dup a! .. ( /) --l- ( /) ! ;
: reading = $23 ( l h) focus @p ! .. ( /) @p @p .. ( /) | , ! ! .. @p ! ; ( /) $1200e , ( /)
: @word = $29 ( -n) @p ! .. ( /) [ $120d9 , ( /) | , ] @p ! @ ; ( /) ] !p .. ( /)
: writing = $2d ( h l n) focus | , left @p ! .. ( /) @p a! .. ( /) | , ! @p ! .. ( /) @p @p @p .. ( /) push push ! | , pop ! pop ! @p ! ; ( /) $1201d , ( /)
: stash = $37 focus @word !p ; = $3a 0 ,

{block 257}
( - old flash bootstream burning helper)
: focus ( helper node gets) # 705 ( to pay attention)
: reading ( tell) # 705 ( to start reading flash)
: @word ( tell) # 705 ( to read) # 18 ( bit word from flash) | , | .. ( using code in rom)
: writing ( tell) # 705 ( to start writing flash)
: stash ( read a word from flash and) | , | .. ( stash it in node) # 706 ( to be read by the ide)

{block 258}
( writing flash) # 8 ( bits) overlay | cr
len ! dest ! source ! | cr
serial load -canon options @ load | cr
a-com sport ! a-bps bps ! !nam ( talk) | , nores 285 list 0 705 hook $0 64 1613 boot | , .erasing pause ( erase) | , ( flash commands) 2 fh load | , | ... dest @ split $1c r! $1e r! $18 call chill | , | , 0 705 hook 0 64 1611 boot | cr
...
: burn ( sdn) | , [ s ( source address of buffer in host memory) | , d ( destination) # 8 ( bit address in flash) | , n ( length of stream in) # 16 ( bit words) | , | ,
: burn ( and)
: check ( are defined at block) # 260 ( for old flash)
: erase ( is done directly ,) | , | .. ( using code from bin) # 1613

{block 260}
( code for reading and writing flash) # 8 ( bits)
: out 0 706 hook ; | *
: in 0 705 hook ;
: read ( a-a') in dup split lit' lit' [ $e ] call | , | .. rdup upd [ -2 ] + ;
...
: burn in [ dest ] @ split lit' lit' [ len ] @ lit' | , | .. left lit' ra! pr out [ source ] @ [ len ] @ | , | .. for 2@+ [ 4 ] * send -next drop ;
: check ( -t) [ dest ] @ read drop | , [ source ] @ [ len ] @ for 2@+ # $39 [ $3b ] call [ $3f ] r@ or | , | .. drop if pop drop fail ; | , | .. ] then -next drop good ;
...
: send ( one word into node) # 705 | , | .. [ pause ( only to display as it runs)
...
: check ( compare nucleus file to flash) | , | , [ the following were used to interactively | , debug this driver , not currently used | ,
...
{block 282}
( streamer) | cr
:var source [0x120f8000] :var dest [0x20000] :var len [0x11ff] | , :var options [0x128] ( default) # 298 ( options !)
: stream ( -an) [ source ] @ [ len ] @ ;
: 18burn ( sdn) # -30 ( fh) [ 252 ] load ;
: newflash ( an) 296 [ options ] ! [ 292 ] load ;
: oldflash ( an) drop drop 298 [ options ] ! ;
: burn ( sdn) # -24 ( fh) [ 258 ] load ;
: brnuc ( sdn) # -24 ( fh) [ 352 ] load ;
: ers ( an) # -32 ( fh) [ 250 ] load ;
: framer # -42 ( fh) [ 240 ] ;
...
len ( length of stream in words) | , options ( two types of flash ,) | , | .. | *
: newflash ( and) | *
: oldflash
: stream -an ( returns) [ source ( and) len ]
: 18burn sdn ( burn flash in) # 18 ( bit words or) | , ( burn newer flash in) # 256 ( byte pages , options)
: burn sdn ( burn old flash in) # 16 ( bit words)
: ers an ( erase flash)
...
: file ( save bootstream as stream.bin)
...
{block 284}
( flash utilties feedback) | , :var result :var 'say [0x102f9999] | , :var 'ask [0x102f98cc] :var 'at [0x102f9c44]
...
( erase flash) | indent
     705 node 1352 load 1613 bin | , | , ( new flash) 1332 load 1340 load
...
{block 292}
( transform from) # 18 ( to) # 16 ( bits in place) macro ]
: !w ( hb) [compile] a! $28966 3, [compile] drop ; forth | , :var cnt [0x40] :var ina [0x4841634] :var outa [0x120ff9f8] | , len ! ( source) dup ina ! a-b outa ! ]
: give [ outa ] @ !w 2 [ outa ] +! ;
: merge ( nn-n) swap $40000 * + ;
: split ( n-nn) [ cnt ] @ /mod [ cnt ] @ 4 * [ cnt ] ! ;
: @18 ( -n) [ ina ] @ @ [ $3ffff ] and 1 [ ina ] +! ;
: nine [ cnt ] @ $40000 or drop if ; ] then | , | .. 4 [ cnt ] ! $ffff and give 0 pop drop ;
: eight [ cnt ] @ $10000 or drop if ; ] then | , | .. 4 * @18 dup push $10000 / 3 and + give | , | .. pop [ cnt ] @ 4 * [ cnt ] ! pop drop ;
: mix nine eight @18 merge split give ;
: transform 0 4 [ cnt ] ! [ len ] @ 9 * 8 / | , | .. for mix next drop ; transform
...
{block 296}
( option to burn newer 25wf080b flash)
: split ( n-lh) dup [ $ffff ] and [ 4 ] * swap | , | .. [ $f0000 ] and [ 64 ] / ;
: eraser ( -bin) [ 1612 ] ;
: writer ( -bin) [ 1612 ] ;
: helper ( -bin) [ 1607 ] ;
: ers ( a) split [ $24 ] r! push [ 4096 4 * ] pop | , | .. [ len ] @ 18 8 */ [ 4095 ] + [ 4096 ] / for | , | .. | .. dup [ $26 ] r! [ $20 ] call chill over + next | , | .. drop drop ;
: erase [ dest ] @ ers ;
: flash [ 350 ] ;
: nucbrn [ 470 ] load ;
: efbrn [ 1192 ] load ;
...
{block 297}
( option)
: erase ( calcs number of 4k byte sectors) | , | , [ 4096 ( is the sector size , which must be) | , ( shifted left) # 2 ( places for 8obits in bin) # 1612
: split # 20 ( bit address to low) # 16 ( bits left) | , ( justified to) # 18 ( bits and high) # 4 ( bits left) | , ( justified to) # 18 ( bits)

{block 298}
( option to burn older 25wf080 flash)
: split ( n-lh) dup [ $ffff ] and [ 4 ] * swap | , | .. [ $f0000 ] and [ 64 ] / ;
: eraser ( -bin) [ 1613 ] ;
: writer ( -bin) [ 1609 ] ;
: helper ( -bin) [ 1610 ] ;
: eras ( a) split [ $1c ] r! [ $1e ] r! [ $18 ] call ;
: erase [ 0 ] eras chill [ $8000 ] eras chill | , | .. [ $10000 ] eras chill [ $18000 ] eras chill ;
: flash [ 254 ] ;
: nucbrn [ 462 ] load ;
: efbrn [ 1194 ] load ;
...
( - older 25wf080 flash)
: split # 20 ( bit address to low) # 16 ( bits left) | , ( justified to) # 18 ( bits and high) # 4 ( bits left) | , ( justified to) # 18 ( bits)
: eras ( does a) # 32 ( chunk for now , temporary fix)
...
{block 348}
( new flash nucleus burning tools) | , :var page [0xa400] :var more [0x800] | ,
: @2 ( b-w) 2@ swab [ $ffff ] and ;
: /page ( nn-) [ more ] @ lit' [ page ] @ lit' [ $23 ] call ;
: get ( a-a'n) dup [ 2 ] + swap @2 ;
: !page ( a-a') 128 for get lit' !a next ;
: burn [ source ] @ [ dest ] @ split [ more ] ! [ page ] ! | , | .. [ len ] @ 9 8 */ 128 / 1 + for | , | .. | .. /page !page $400 [ page ] +! next | , | .. drop ;
: read ( a) left lit' ra! | , | .. split swap lit' lit' [ $2c ] call ;
: fetch ( -n) [ $36 ] call [ $39 ] r@ ;
: check ( -t) [ dest ] @ read | , | .. [ source ] @ [ len ] @ 9 8 */ | , | .. for get fetch $ffff and | , | .. | .. over over or drop if pop fail ; ] then | , | .. | .. drop drop next drop good ;

{block 349}
( new flash nucleus burning tools) | , page ( high byte of) # 16 ( bit address) | , | .. ( left justified) | , more ( remaining bits above page) | , | .. ( also left justified) | ,
: @2 ( fetch) # 16 ( bit word from byte address) | , | .. ( no need to invert bits in this case) | , | .. ( but bytes need to be swapped) | ,
: /page ( issue the) [ wrpg ( instruction) | ,
: get ( read the next) # 16 ( bit word of nucleus) | ,
: !page ( read and send the next) # 256 ( bytes) | ,
: burn ( read from source and write to flash) | , | .. ( the whole nucleus) | ,
: read ( initiate a read instruction at) | , | .. ( the) # 24 ( bit address , or) # 20 ( bit address)
: fetch ( read next word from flash to pc stack)
: check ( compare nucleus file to flash)

{block 350}
( - new flash bootstream burning tools) | , :var page [0x1e800] :var more
: @2 ( b-w) 2@ [ $5555 ] or [ $ffff ] and ;
: /page ( nn-) [ more ] @ lit' [ page ] @ lit' [ $23 ] call ;
: get ( a-a'n) dup [ 2 ] + swap @2 ;
: !page ( a-a') 128 for get ( pause) lit' !a next ;
: burn [ source ] @ a-b 0 [ page ] ! 0 [ more ] ! | , | .. [ len ] @ 9 8 */ 128 / 1 + for | , | .. | .. /page !page $400 [ page ] +! next | , | .. drop ;
: read ( a) left lit' ra! dup $f0000 and 64 / | , | .. swap $ffff and 4 * lit' lit' [ $2c ] call ;
: fetch ( -n) [ $36 ] call [ $39 ] r@ ;
: check ( -t) [ dest ] @ read | , | .. [ source ] @ a-b [ len ] @ 9 8 */ | , | .. for get fetch $ffff and | , | .. | .. over over or drop if pop fail ; ] then | , | .. | .. drop drop next drop good ;

{block 351}
( - new flash bootstream burning tools) | , page ( high byte of) # 16 ( bit address) | , | .. ( left justified) | , more ( remaining bits above page) | , | .. ( also left justified) | ,
: @2 ( fetch) # 16 ( bit word from byte address) | , | .. ( invert bits in this) | , | .. ( but no need to swap bytes) | ,
: /page ( issue the) [ wrpg ( instruction) | ,
: get ( read the next) # 16 ( bit word of bootstream) | ,
: !page ( read and send the next) # 256 ( bytes) | ,
: burn ( read from source and write to flash) | , | .. ( the whole bootstream) | ,
: read ( initiate a read instruction at) | , | .. ( the) # 24 ( bit address , or) # 20 ( bit address)
: fetch ( read next word from flash to pc stack)
: check ( compare bootstream buffer to flash)

{block 352}
( writing new flash nucleus) overlay | cr
len ! dest ! source ! | cr
serial load -canon options @ load | cr
a-com sport ! a-bps bps ! !nam ( talk) | , nores 285 list 0 705 hook $0 64 eraser boot | , .erasing pause | , ( commands flash) 348 load | , erase chill | , | , 0 705 hook $0 64 writer boot focus | cr
0 706 hook $0 64 helper boot | , | cr
285 list nosay ( nores) | cr
.burning pause burn chill | cr
.checking pause check | , ( spi ready) 0 705 hook 0 64 1606 boot 4 call | cr
overlay
...
: ersnuc ( does the appropriate erase) | , | .. ( for new or old flash depending on) [ options
: burn ( and)
: check ( are defined at block) # 348 ( for new flash)

{block 354}
( code for reading and writing new flash nucleus)
: split ( n-lh) dup [ $ffff ] and [ 4 ] * swap | , | .. [ $f0000 ] and [ 64 ] / ;
: out 0 706 hook ; | *
: in 0 705 hook ;
: read ( a-a') in dup split lit' lit' [ $e ] call | , | .. rdup upd [ -2 ] + ;
...
( - load descriptors) | ,
: l, ( nn) dup +node 100 /mod 1 and 2* swap 1 and + 1714 + push 32 32 pop /part ( warm) $a9 /p ;
: ganglia nns 2 * for i -1 + n-nn l, next ; | , | , ( sea of mk1) ganglia | , ( snorkel mk1) 207 +node 1605 /ram up /b $37 /p | , | , ( sram cluster mk1) sram 2 + load | , ( virtual machine) 2 fh load | , ( serial terminal) 4 fh load | , ( additional i/o) 6 fh load | , ( application code) # 1082 ( load) | , ( clock nodes exit) | , | .. 617 +node 617 /ram io /b $200 /p | , | .. 616 +node 616 /ram left dup /a /p right /b | , | .. 517 +node 0 /io 417 +node 0 /io | , ( ethernet cluster mk1 exit) | , | .. ether 2 + load
...
{block 368}
( -- additional i/o) | , ( spi) 705 +node 1606 /ram io /b # $a9 4 /p | , ( async bootable) 708 +node $aa /p
...
{block 402}
( - interface) 104 node $20 org | , ( programmed with abandon)
: start io b! right a! ahead
: rx? = $24 $400 and .. if ( avail) up a! @ then
: rtn ( w-) = $29 $ffff and right a! ! | ,
: idle = $2d then @b 2* .. [ ' rx? ] -until | , | ... ( not read) 2* [ ' idle ] -until | , | ... ( write) @ 2* 2* 2/ 2/ .. -if
: tx! ( c-) = $33 @b $1000 and if ( full) drop rtn ; | , | ... ( room) ] then left a! over ! rtn ;
: iox ( c-) = $3a then left a! ! | , | ... up a! begin @ - -until - rtn ; | , = $40 1708 bin
...
{block 404}
( generate ganglia) | , | , 700 node $20 org reclaim | , ( rlud) ] r--- --l- ---u -d-- [ 2 fh load | , 1716 bin = $3f | , | , 701 node $20 org reclaim | , ( rlud) ] --l- r--- ---u -d-- [ 2 fh load | , 1717 bin = $3f | , | , 600 node $20 org reclaim | , ( rlud) ] r--- --l- -d-- ---u [ 2 fh load | , 1714 bin = $3f | , | , 601 node $20 org reclaim | , ( rlud) ] --l- r--- -d-- ---u [ 2 fh load | , 1715 bin = $3f
...
{block 406}
( - ganglion mark) # 2 | , | , # $20 ( org rlud r--- --l- ---u -d--)
: aim = $24 ( p-pa) dup 3 and $20 or b! @b dup b! !b | , | .. @p ; .. ( pump) ] leap
: whither = $2a ( p) -4 . + aim !b | , ( cnts) = $42e pop !b pop !b | , ( path) = $82f .. ahead begin drop @ [ swap ] then | , | .. dup !b -until
: payload = $32 begin @ !b unext | , | ... begin @b ! unext ;
: pump = $34 then pop a! | , ( cnts) @ dup push @ dup push push push | ,
: turn = $37 begin @ dup $ffc and if | , ( next leg) drop whither ; ] then drop -until | , ( deliver drop) aim pop ( drop) pop ( drop) payload ; | , = $3f

{block 407}
( improved ganglia deliver exchanges of) # 1 ( or) | , ( more words out and) # 1 ( or more word reply to any arbitrary nodes with more flexible source) | , ( routing than did mark) # 1 ( and no gotchas.) | , ( message structure...) | , | , | .. ] focus call ( to port) [ always there | , | .. | .. ] pump call [ interganglion only | , | .. | .. ] reply count ( words-1) [ inter only | , | .. | .. ] payload count ( words-1) [ inter only | , | .. | .. ] path ( see below) [ inter only | , | .. ] payload [ always there | , | .. ] reply [ always there | , | , ( path is a sequence of) # 1 ( or more words with one) | .. ( run in each, encoded as follows...) | , | .. | .. ( e- ---- nnnn nnnn nndd) | , | .. ( d is cardinal direction ewns 0123) | , | .. ( n is count 1-relative zero deliver immed) | , | .. ( e marks last run) | , | , ( when an exchange is finished all) | , ( ganglia are back where they were) | , ( on receipt of focusing call.)
...
: out! = $33 out null ;
: slob = $35 ( d-dw) dup dup or
: +slob = $36 ( dw-dw) 7 push ibits ;
: slowin = $39 ( d) @ for slob zif ! done ; | , | ... ] then +slob ! next done ; | , = $40 1606 bin reclaim
...
: out! ( given nbyt-1 sends to dev, ends + desel.) | , | ,
...
{block 414}
( - ganglion mark) # 1 | , | , ( a whence b whither) | , ( msg is focus call path cin-1 cout-1 code) | , | , # $20 ( org rlud r--- --l- ---u -d--)
: aim = $24 ( p-pa) dup 3 and $20 or b! @b dup b! !b | , @p ; ( / pump) ] leap
...
{block 420}
# 2 ( chip async bootstream demo) | , empty compile streamer load framer load | , ( async) | *
: frame [ $ae ] fram ; 2chip | , ( the stream) 2 fh load stream ( -an) | , | , serial load -canon # 0 ( fh orgn !) | , a-com sport ! a-bps bps ! !nam | , 2chip talk send | , | , ( new ide path) 220 load !b0 | , 0 10600 hook panel

{block 421}
( - load screen for pf boot stream via async) | , | , panel ( is just to set us know it's finished) | , | , ( the ide is left working so make a new) # 2 ( chip) | , ( ide path and hook to node) # 10600 ( to prove) | , ( it works)

{block 422}
( - generate stream)
: brdg align create [ 708 700 to 600 300 to -1 , | , brdg course | , 708 +node 1600 /ram $ae /p | , 500 +node 1904 /ram 5 /a io /b down /p | , 400 +node 1907 /ram down /b up /a 0 /p | , 300 +node 1901 /ram $19 /p | , frame | , | ,
: app align create [ 708 700 to 600 400 to | , | .. 10400 10600 to 10601 10616 to | , | .. 10617 10717 to -1 , | , fresh app course | , 708 +node 1600 /ram $ae /p | , frame | ,

{block 423}
( -)
: sram1 ( path just to load sram driver and data) | , | , ( read nucleus to get word count from address) # 0 | , | , ( construct a boot header by hand to send the) | , ( nucleus down to loader in node) # 108 ( ,) | , ( commented exit would allow reading and writing sram via the ide for testing) | , | , ( rest of nodes are loaded) | , | , ( note if you want to use the ide after sending you need to load bin) # 1600 ( into node) # 708

{block 424}
( async pf bootstream with bridge) | , empty compile streamer load framer load | , ( async) | *
: frame [ $ae ] fram ; 2chip | , ( the stream) 2 fh load stream ( -an) | , | , serial load -canon # 0 ( fh orgn !) | , a-com sport ! a-bps bps ! !nam | , 2chip talk send | , | , ( new ide path) 220 load !b0 !b2 | ,
: bri1 align create | , [ 708 608 to 607 600 to 500 400 to | , 10400 10500 to 10501 10507 to 10508 10708 to | , -1 , ]
: !b1 bri1 [ 'pths 1 + ] ! ; !b1 | , # 0 # 10600 ( hook) panel

{block 425}
( - load screen for pf boot stream via async) | , | , ( build a) # 2 ( chip bootstream rooted at node) # 708 | , ( leaving address and count on stack) | , | , ( load serial ide and modify it to send a stream straight to the serial node) | , | , ( reset host via) talk ( and) send ( the stream) | , | , ( the ide is left working so make a new) # 2 ( chip) | , ( ide path and hook to node) # 10600 ( to prove) | , ( it works)

{block 426}
( - generate pf stream) | , | ,
: sram1 align create | , | .. [ 708 108 to 107 100 to 0 9 to -1 , | , sram1 course ( sram driver) sram 2 + load | , ( odd one) 0 +node | , ( ide) 708 +node 1600 /ram $ae /p | , ( sram loader) 436 load frame | , ( read nucleus) 434 load @nuc | , ( header) $100ae !18 down !18 fbuf t@ 2 + !18 | , | .. drop !nuc ( exit to test sram working) | , | , ( bridge to target)
: brdg align create [ 708 701 to 700 300 to -1 , | , fresh brdg course | , 708 +node 1600 /ram $ae /p | , 400 +node 1907 /ram down /b up /a 0 /p | , 500 +node 1904 /ram 5 /a io /b down /p | , 300 +node 1901 /ram $19 /p frame | , | , ( path covering both chips) 2 fh load | , fresh entire course | , ( rest of chip) pf 2 + load | , ( ide) 708 +node 1600 /ram $ae /p frame | ,

{block 427}
( -)
: sram1 ( path just to load sram driver and data) | , ( node) # 0 ( must be carefully loaded along with) | , ( sram or not reachable) | , | , ( read nucleus to get word count from address) # 0 | , | , ( construct a boot header by hand to send the) | , ( nucleus down to loader in node) # 108 ( ,) | , ( commented exit would allow reading and writing sram via the ide for testing) | , | , ( rest of nodes are loaded) | , | , ( note if you want to use the ide after sending you need to load bin) # 1600 ( into node) # 708

{block 428}
( -- path both chips)
: entire align create [ 708 716 to 717 17 to | , | .. 16 10 to 110 116 to 216 209 to 109 108 to | , | .. 208 207 to 307 316 to 416 407 to | , | .. 507 516 to 616 607 to 707 706 to | , | .. 606 6 to 5 705 to 704 4 to 3 103 to | , | .. 102 2 to 1 101 to 100 200 to 201 203 to | , | .. 303 703 to 702 302 to 301 701 to 700 400 to | , | .. 10400 10700 to 10701 10717 to 10617 10601 to | .. 10501 10517 to 10417 10401 to 10301 10317 to | .. 10217 10200 to 10100 10117 to 10017 10000 to | .. -1 ,

{block 429}


{block 430}
( install pf via async bootstream) | , empty compile streamer load framer load | , ( async) | *
: frame [ $ae ] fram ; | , ( the stream) 2 fh load stream ( -an) | , | , serial load -canon # 0 ( fh orgn !) | , a-com sport ! a-bps bps ! !nam | , talk send panel | , | , ( test) exit ]
: s1+ ( nm-mn') swap [ 1 ] + ; ( sm) 454 load ]
: spa align create [ 708 108 to -1 , | , spa 'pths 1 + ! 1 108 hook panel

{block 431}
( - load screen for pf boot stream via async) | , | , panel ( is just to set us know it's finished)

{block 432}
( - generate pf stream) | , | ,
: sram1 align create | , | .. [ 708 108 to 107 7 to 8 9 to -1 , sram1 course ( sram driver) sram 2 + load | , ( ide) 708 +node 1600 /ram $ae /p | , ( sram loader) 4 fh load frame | , ( read nucleus) 2 fh load @nuc | , ( header) $100ae !18 down !18 fbuf t@ 2 + !18 | , | .. drop !nuc ( exit to test sram working) | , | , fresh sram 4 + load s708 course | , ( rest of chip) pf 2 + load | , ( ide) 708 +node 1600 /ram $ae /p frame | ,

{block 433}
( -)
: sram1 ( path just to load sram driver and data) | , | , ( read nucleus to get word count from address) # 0 | , | , ( construct a boot header by hand to send the) | , ( nucleus down to loader in node) # 108 ( ,) | , ( commented exit would allow reading and writing sram via the ide for testing) | , | , ( rest of nodes are loaded including) # 600 ( which) | , ( toggles its pin for the scope , just a demo) | , | , ( note if you want to use the ide after sending you need to load bin) # 1600 ( into node) # 708

{block 434}
( -- read pf nucleus) :var ft :var chr [0x2400] 0 chr ! ]
: a-b ( a-b) 2* 2* ; | *
: b-a ( b-a) [ 3 ] + 2/ 2/ ;
: allot h +! ; | *
: buffer pop ;
: fbuf align buffer [ 9216 allot ]
: +pf [ 0 fnam b-a ] r/o fopen if [ ft ] ! ; | , | .. ] then drop abort ;
: -pf [ ft ] @ [ 0 ] + if fclose 0 then [ ft ] ! ; | , | , macro | ,
: 2@ ( b-h) $8b66 3, ; | *
: swab ( h-h) $c486 2, ; forth ]
: t@ ( a-an) dup 2@ swab $ffff and ;
: @nuc +pf [ fbuf ] 9216 [ ft ] @ frd [ chr ] ! -pf ;
: !img ( nc) push [ fbuf ] begin t@ !18 [ 2 ] + | , | .. next drop ;
: !nuc [ fbuf ] t@ [ 2 ] + !img drop ; | , | , named ( sf/pfdisk.blk)

{block 435}
( - nucleus in file pf/pfdisk.blk is read into a buffer on the host ,) | , | , !img ( uses) ] t@ ( to transform nucleus into correct byte order and bits per word while sending it through the serial interface via the ide's !18)

{block 436}
( -- load polyforth nucleus) | ,
: /nwire ( in out) /b /a 0 4 [ 1711 ] /part 0 /p ; | , 608 +node down up /nwire | , 508 +node up down /nwire | , 408 +node down up /nwire | , 308 +node up down /nwire | , 208 +node down up /nwire | , 108 +node 1713 /ram left /b 0 /p

{block 437}
( - wire passes word count and data down) | , ( to loader in node) # 108

{block 438}
( install pf w/bridge in flash) | , | , empty ( 1st pass) compile streamer load | , framer load 2chip ( the stream) 2 fh load | , | , empty ( 2nd pass) compile streamer load | , framer load 2chip ( the stream) 2 fh load | , stream ( oldflash) newflash | , | , .stream | , ( write stream into flash) | , ( display stream length) strlen leng ! | , stream 0 swap 18burn | , ( ask permission to burn nucleus as well)
: esc key? esc ;
: ?cont esc [ keych ] @ 0 [ keych ] ! | , | ... [ $7f ] and [ $20 ] or drop if noask exit ; ] then ; | , .cont pause ?cont | , serial load -canon options @ load | , | ... ( erase and burn nucleus) nucbrn
...
{block 440}
( - generate pf stream w/ bridge) | , ( paths) 2 fh load | , ( speedup) | , | ... 0 0 3 1608 /root | , sram1 course | , | ... ( sram driver) sram 2 + load | , | ... ( flash to sram) 468 load | , | ... frame 1adjust ( resume point) | , fresh brdg course | , | ... 400 +node 1907 /ram down /b up /a 0 /p | , | ... 500 +node 1904 /ram 5 /a io /b down /p | , | ... 300 +node 1901 /ram $20002 /io $19 /p | , | ... 705 +node 1812 /ram $2f /p | , | ... frame 2adjust ( point adjust) | , fresh ( residual) sram 4 + load sb705 course | , | ... ( rest of chip) pf 2 + load | , | ... ( test) # 10500 ( +node) # $30000 ( /io) | , | ... ( test) # 10100 ( +node) # $30000 ( /io) | , | ... ( + chip select) # 705 ( +node) # 1710 ( /ram) # $28 ( /p) | , | ... frame
...
{block 442}
( paths)
: sram1 align create [ 705 605 to 606 607 to | , | .. 608 208 to 108 107 to 7 9 to -1 , ]
: brdg align create [ 705 708 to 608 600 to | , | . 500 300 to -1 , ]
: sb705 align create [ 705 717 to 617 17 to | , | . 16 10 to 110 108 to 208 206 to 306 309 to | , | . 209 210 to 310 311 to 211 111 to 112 116 to | , | . 216 212 to 312 316 to 416 406 to 506 516 to | , | . 616 605 to 505 205 to 105 106 to 6 5 to | , | . 4 704 to 703 3 to 2 102 to 101 1 to 0 200 to | , | . 201 202 to 302 301 to 401 402 to 502 501 to | , | . 601 602 to 702 701 to 700 400 to | , | . 10400 10700 to 10701 10717 to 10617 10601 to | , | . 10501 10517 to 10417 10401 to 10301 10317 to | , | . 10217 10200 to 10100 10117 to 10017 10000 to | , | . -1 ,
...
: resume [ @res2 ] @ $fffff and 18 8 */ 4 * lit ; | , target ]
...
: start = $19 down a! # 0 # $20000 $800 $0 fet | cr
16ibits dup ! for 16ibits ! next | indent
     resume fet 18ibits $1e000 . + - -if
: done = $28 io b! 20 20 -++ half warm ; ] then
: /frm = $2f 5 resume push spicmd spi-boot ; | cr
= $34 # 1710 1812 bin

{block 445}

: start ( 1st word read is count in words) | , ( for loading a pf nucleus) | , | ,
: /frm ( initiates the next boot frame in) | , ( a multi frame boot stream)
...
{block 448}
( - spi flash) # 8 ( bits) 705 node 0 org host ]
: resume [ @res1 ] @ $fffff and 18 8 */ 4 * lit ; | , target ]
: wait ( dw-dw) = $0 select dup begin drop @b -until drop select ;
: 2cmd ( dw-d) = $4 select 8obits
: cmd ( dw-d) select
: !8 ( dw-d) 8obits drop ;
...
{block 450}
( polyforth ide boot) host ( bridge) load | , | ... loader load | , ( kill boots) 0 708 hook 0 -hook ]
...
: !nuc [ chr ] @ 2/ [ 1 ] + !img ; | , | ... 1 108 hook ( killme) !nuc | . unfoc 1 -hook | , | , ( rest) pf 2 + load | . 2 ship | . panel upd ?ram | ,
...
framer load ( the stream) 466 load | , stream ( oldflash) newflash | , | , .stream | cr
...
{block 461}
( pf stream) | , | , ( two passes required to calculate address) | , ( to resume booting from after loading sram) | , ( and compile it into node) # 705 | , | ,
: oldflash ( sets options to sst2508 flash)
: newflash ( sets options to winbond 25128 flash)
: 18burn ( depends on options , burns bootstream)
: nucbrn ( depends on options , burns nucleus)

{block 462}
( - burn pf nucleus old flash) | , empty compile streamer load | , .nucleus nores | , | , ( read pfdisk.blk) 464 load | , | ... fbuf source ! 4607 len ! | , ( display nuc length) | , | ... len @ 1 + 2 * 8 18 */ leng ! | , | , | ... ( burn nucleus) stream $20000 swap 258 load

{block 463}
( burns nucleus to flash starting at) | , ( 128k , x20000)
...
: @nuc +pf [ fbuf ] 9216 [ ft ] @ frd [ chr ] ! -pf ; | , | , named ( sf/pfdisk.blk) | , | , @nuc

{block 465}
( -- read pf nucleus) | , ft ( file handle) | , chr ( number of characters read)
: a-b ( word address to byte address)
: b-a ( byte address to word address)
: fbuf ( allot a file buffer)
: +pf ( open the nucleus file)
: -pf ( close the nucleus file)
: @nuc ( read nuleus into file buffer)
...
( sram driver) sram 2 + load | , ( flash to sram) 2 fh load | , frame 1adjust ( resume point) | , | , fresh ( residual) sram 4 + load s705 course | , | ... ( rest of chip) pf 2 + load | , | , frame

{block 467}
( - generate pf stream) | , | , 1adjust ( there are two resume points now) | , | .. ( this one used to resume the bootstream) | , | .. ( after load the nucleus from another place) | , | .. ( in flash)
...
{block 470}
( - burn pf nucleus new flash) | , empty compile streamer load | , .nucleus nores | , | , ( read pfdisk.blk) 464 load | , | ... fbuf source ! 4607 len ! | , ( display nuc length) | , | ... len @ 1 + 2 * 8 18 */ leng ! | , | , | ... ( burn nucleus) stream $20000 swap 352 load
...
{block 638}
# 1904 ( ats sync bridge) 646 load exit | cr
...
: -rst $20000 set ; | *
: +rst $30000 set ;
: frame then -rst 1000 for next | , | .. +rst # 1000 ( for next) | , @s @s @s dup -1 . + push push push | , !his pop !his pop !his begin @s !his next
: local @p !b [ .. dup or a! .. ] @p ! [ .. off .. | , ] @p ! [ .. dup or a! .. ] $3f for @s !ours next | , @p ! [ .. go ; .. ] @p !b [ .. warm ; .. ] warm ; | , = $2e 1907 bin

{block 645}
( bridge loader) | , ( node) # 300 ( contains code used by) # 400 ( to boot) | , ( node) # 300 ( of the target chip .) | , | , ( node) # 500 ( contains code to be loaded into both nodes) # 300 ( .) | , | , ( node) # 400 ( contains code that moves code) | , ( from) # 500 ( into both nodes) # 300 ( .) | , | , ( node) # 401 ( has test code that toggles an io pin)

{block 646}
# 1904 ( new!!! ats sync bridge) | , reclaim 300 node # 3 8 org | , host | *
: hd- # -3 -8 + ; target | *
: orgn | , | ,
: ?lo = $8 begin @b - -until ;
: dly = $a ( b !b) 40 for unext !b ; ( 88ns)
: 1bt = $d ( wb-w') dup dly $10000 or dly ;
: zro = $11 $10001 dly ; | *
: wpd = $13 $10001 !b ;
: 18o = $15 ( w zro) 17 for begin | , = $17 -if $30003 1bt 2* *next drop wpd ; | , = $1c ] then $30002 1bt 2* next drop wpd ; | , | ,
: 18i = $20 ( hi-z) dup or !b 17 for | , = $23 begin @b -until | , = $24 begin @b - -until | , = $25 - 2 and 2/ a 2* or a! next | , = $2a a up a! ! ( ack) $30001 1bt wpd | , | ,
: idl = $30 $165 ( --lu) a! . @ @b -if drop 18i ; | , = $34 ] then ( zro) drop 18o begin @b -until ?lo idl ; | ,
: ent = $38 io b! wpd ?lo idl ; | , = $3d here here hd- # 0 5 org ]
: frame ent orgn [ , | , org = $3d 1904 bin

{block 647}
( this code is loaded into nodes) # 300 ( of two) | , | .. ( chips to make their up ports into a slow but) | .. ( transparent bridge between the chips.) ] this | , | .. version includes flow control. | , | , ( each node waits for port data or rising clock edge and based on clock state after wakeup) | , ( moves a word between sync serial and port.) | , | , ( on the line, xmtr controls clock and data are sampled on falling edge. clock and data set to weak-pd immed after last fall. xmtr waits for ack from rx ... which delays 1/2bt then clock hi for 1/2bt and low with immed weak-pd.) | , | , ( thus,) there is flow control in this version, | , | .. ( but read handshake line of the up port is) | , | .. ( not meaningful.) | , | , [ origin moved to 5 so boot frame header can be laid down without affecting slot 2 jumps.
...
{block 670}
( tb001 ide creepers) empty serial load | , ( customize) -canon 670 orgn ! | , a-com sport ! a-bps bps ! !nam | , ( functions) 674 5 loads | , | , ( test status is) | .pok | , | , ( details) :var ph [0x3] :var id [0x7d2] :var tgt [0x3e8] :var pos [0x9a] | , ( -------) :var ans :var rval [0x708a] | , ( -------) :var nrun [0x3345] :var vval [0x1] | , | ,
...
compile serial load -canon :var usb [0x9] | cr
...
{block 720}
( 10baset ethernet cluster mk1) | , | , ( tx) 8 fh 24 fh thru | , | .. ( naked xtal) 72 fh 76 fh thru | , ( dma) # 28 26 fh 32 fh thru | , | .. ( tx ctl) 34 fh load | , ( rx) 36 fh 52 fh thru | , | , ( clock monitor) 54 fh 56 fh thru

{block 721}
( software-defined / bitbanged dma controller) | , ( for full duplex) # 10 ( megabit ethernet.) | , | , ] mark 1 ( - does autonomous link state management so it's usable by low level applications. for exclusive use by high level such as polyforth this could be simplified and it would save one spinning node.)
...
{block 726}
( - tx load descriptor) | , | , ( tx) 417 +node 417 /ram $2 /p 0 /io | , | .. 317 +node 317 /ram $20000 $30000 over over | , | .. | .. over over over over over over 10 /stack | , | .. | .. io /b up /a 4 /p | , | .. 316 +node 316 /ram left /a right /b 0 /p | , | .. 315 +node 315 /ram left /b # $c $10 /p | , | .. 215 +node 215 /ram up /a down /b $9 /p | , | .. 115 +node 115 /ram down /a io /b 7 /p | , | .. 314 +node 314 /ram down /a right /b $a /p | , | .. 214 +node 214 /ram up /a down /b $27 /p | , | .. 114 +node 114 /ram left /a up /b $20a /p | , | .. 113 +node 113 /ram right /a left /b $a /p | , | .. 112 +node 112 /ram left /a right /b 0 /p | , | .. 111 +node 111 /ram right /b left /a $16 /p | , ( naked 10mhz xtal) exit 0 io right 517 +nd | , | .. ( ram) 0 right down 516 +nd | , | .. ( pat) 0 down left 416 +nd | , | .. ( strt) 0 left 0 415 +nd

{block 727}
( the clock/monitor code is loaded as part of pf hence it is under an exit here.)

{block 728}
# 417 ( resonator tx osc) | , 788 load exit 417 node 0 org | , | ,
: drive = $0 ( 4n) !b !b for unext drop ;
: init = $2 up a! @ drop | . io b! left a! | , 8 dup 0 $20000 9 dup $800 $30000 | , | ... dup dup drop drop 9 for drive drive next | , $15555 up 0 $20000 | . $15555 up $800 $30000 | , | ... dup dup drop drop = $422
: go = $22 . @ drop . !b !b a push a! ! pop a! go ; = $26 reclaim exit

{block 729}
# 10 ( mhz ceramic resonator drive.) | , ( each) # 50 ( ns edge sends a return instruction) | , ( thru up port to tx pin node) # 317 ( which waits) | , ( for the edge by simply calling the port.) | , io power seems to be 70 to 90 uamps. | , | , ( node) # 317 must ( wait faithfully for) every ( edge) | , ( to keep this oscillator running reliably.) | , | ,
...
: /slp = $3 @p !b ; ] .. slp ;
: /flp = $5 @p !b ; ] .. flp ;
: /-lnk = $7 ( 1.25 sec) 20000 for /flp next ; | , | ,
: kall = $c ( a) dup ( a!) !who push ex pop dup push !who ;
: run = $10 ( 'rd-- a! @ drop) io a! @ dup push = $413 | , | .. $2000 and if '-d-- kall then = $17 | , | .. pop 2* 2* -if 'r--- kall then run ; = $1c
...
{block 733}
( multiplexor for command of node node) # 315 | , ( by) # 314 ( or 215.) | , spins, will use smart io read when available. this loop is costing 3.4-3.5 ma. | , | , ( to claim the node start writing code to its) | , ( port; no focusing call needed, and a will be) | , ( pointed at the port in node 315. you then own it until you send it a return instruction or) | , ( equivalent. code may call these routines...) | , | ,
: /slp ( generates a slow link pulse)
: /flp ( delays 62.5us)
: /-lnk ( delays 1.25 sec with no link pulses,) | , | .. ( thus forcing renegotiation)
...
: -lnk = $5 @p !b ; ] .. /-lnk
: wd ( n) = $7 @p !b !b ; ] .. @p /wd
: auto = $9 begin @ dup !link | , | .. | .. dup # $41 $4041 and # $41 $4041 or until | ,
: neg = $f -lnk 4 for $41 wd next | , | .. | .. 7 for $4041 wd next -attn | , | .. 60000 # 20000 ( n.n ms) for | , | .. | .. 41664 for unext next auto ; = $21 | , | , 115 node 0 org ]
: !link ( n) = $0 @p drop !p ; | *
: link ( -n) = $1 -1 ;
: pass a up a! link ! a! ;
...
{block 735}
( node) # 115 ( passes new link states from 015 to) | , # 215 ( without blocking 015 ever. suspends till) | , ( new state is available then spins on io to) | , ( deliver it to 215. new state may be received) | , ( while spinning and this simply replaces the) | , ( state we are trying to deliver, so that when) | , # 215 ( gets around to reading it will see the) | , ( most recent link state received.) | , | , ] node 215 ( takes latest state from) # 115 ( and checks it for ack of 802.3 10fd mode. if so, we are happy and do nothing. otherwise we perform one negotiation cycle, open loop, by sending) # 5 | , ( code words of 10fd,) # 8 ( of ack 10fd, and then) | , ( ignoring new link states for six seconds to) | , ( let the other end make up its mind. at the end of that time we expect to receive a good ack) | , ( from node 115.) # 215 ( is not required to be timely in reading from 115.)
...
: +zer = $6 @p !b ; ] .. /zer
: +idle = $8 @p !b .. /idle ; ( ...)
: run = $a @ -if ( attn) -lnk run ; ] then ( attn)
: pre = $d 30 for +one +zer next +one +one
: pkt = $14 @ -if +idle then | , | .. = $16 if +one pkt then | , | .. = $19 +zer pkt | , = $1b [ reclaim
...
: pre ( generates ethernet preamble of) # 61 ( bits) | , ( alternating 1/0, start frame delimiter of two consecutive 1's, all bits of packet including crc, and the end of frame signal.)
...
: prv ( shows two consecutive same states.) [ we need | .. a better noise filter than this some day. ]
...
: run = $5 edge a! @b ! ( orig) # 4 ( best-42) # 5 | , | ... ( best-145) 9 push a! | , | ... begin unext run ; = $a | , | , # $17 ( buffers for) # 16 ( and converts bits to 1/0) | , 17 node 0 org +cy | ,
...
{block 774}
# 617 ( tx osc monitor) 617 node reclaim | , 0 org +cy ]
...
{block 776}
# 616 ( tx osc agent) 616 node 0 org +cy | , | ,
: @tfre = $200 @p .. ask ; | * | , | ..
...
{block 788}
# 417 ( passive tx osc) 417 node 2 org | , | ,
: init = $2 up a! @ drop | . io b! left a! | , | .. left $15555 up $0 # $10000 | , | .. left $15555 up $800 # $10800 | , | ... dup dup drop drop = $813
: go = $13 . @ drop . !b a! ! a! go ; | , = $16 reclaim exit

{block 789}
( fox 819a-10) # 10 ( mhz oscillator input.) | , ( each) # 50 ( ns edge sends a return instruction) | , ( thru up port to tx pin node) # 317 ( which waits) | , ( for the edge by calling or reading the port.) | , | , ( node) # 317 must ( wait faithfully for) every ( edge) | , ( for compatibility with soft oscillators.) | , | ,
: note ( node) # 317 ( starts us when it is ready to go by writing to the port.) | ,
...
{block 792}
# 517 ( xtal oscillator node) | , reclaim # 10715 517 node 0 org ]
: -osc ( -t) = $0 right a! | , | .. $20000 $30000 = $4 7 for over unext = $807 | , | .. @ for = $8 !b @ push begin unext next | , ( sense) = $a ( wpd) 15555 !b | , | .. $30000 for = $e @b - .. -while next ; = $12 | , | .. | .. ] then pop dup or ; | , | ,
: valz = $13 $0 $20000 $800 $30800 ; | , | ,
: fire = $19 -osc dup ( down) right a! if dup or ! ; | , | .. ( continue... then) # 1 ( ! ; exit) | , | .. ( go...) ] then -1 ! valz valz ( up) left a!
: sync = $24 $3ffff 0 !b @ drop for = $28 | , | .. !b !b @ drop next ( @b) # $2000 ( and if ;) | , ( then drop) sync ; | , = $2b ( reclaim) exit

{block 793}
this node excites the crystal, detects ring, | , | .. and syncs to run oscillation as commanded. | , | , ( a param node, monitor node or pin) | , ( b points to io) | , | ,
: -osc ( pumps energy into crystal using sequence) | , | .. ( of half-cycle periods fed from param node.) | , | .. ( then senses rise indicating ring within time) | .. ( out period on order of) # 4 ( ms. true if no ring) | ,
: fire ( called externally to attempt start-up) | , | .. ( with pattern chosen in param node. returns) | , | .. # 0 ( for failure,) # 1 ( for ring but not started,) | , | .. # -1 ( for ring and start. select no start by) | , | .. ( un-commenting the 'continue...' line.)

{block 794}
( -) # 416 ( oscillator sequence gen) | , ( reclaim) # 10713 416 node 0 org | ,
: /rewind = $0 @p !b ; ] .. dup or a! ..
: /push ( n) = $2 @p !b !b ; ] .. @p push ..
: /read = $4 @p !b ; ] .. begin @p !+ unext ..
: !osc ( n) = $6 @p !b !b ; ] .. @p !b ..
: /write = $8 @p !b ; ] .. begin @+ !b unext ..
: @osc ( -n) = $a @p !b @b ; ] .. @b !p .. | ,
: bres ( so-sov) = $c over . + -if a ; | , | .. ] then = $f -64 . + a 1 . + ; ( o init) # -33 ( s pos)
: 'step ( sub val rep -r) = $14 push /rewind | , | .. = $15 a! 63 dup push /push /read | , | .. | .. = $19 -33 begin bres !b next = $1d | , | .. = $1d @p !osc [ .. ] fire .. | , | .. = $1f pop dup !osc /push /write @osc ; | , = $24 ( reclaim) exit | ,

{block 795}
] p ( control node;) b ( is ram node) | , | , /rewind ( set a in data node to) # 0 | , /push ( push value on data node return stack) | , /read ( fill data node memory) | , !osc ( writes value to xtal node's port) | , /write ( send data to xtal node) | , @osc ( reads value from xtal node's port) | , | , bres ( returns value for next half cycle. during) | .. ( interpolation) s ( is positive /64 fraction,) | , | .. t ( is ramp var,) a ( is the integer value.) | , 'step ( given /64) [ sub, ( base) val, ( count) rep ( of) | , | .. ( halfcycles-1, loads pattern into ram node,) | , | .. ( calls) ] fire ( in xtal node, feeds it pattern of) | .. [ val ( and) val+1, ( returns result from xtal.)

{block 796}
( -) # 415 ( osc start + take data) | , ( reclaim) # 10712 415 node 0 org ]
: strt = $0 leap await ; | , | ,
: trial ( n-r) = $2 @p !b [ .. ] @p @p @p .. | , | .. dup 63 and !b 2/ 2/ 2/ 2/ 2/ 2/ !b | , | .. | .. ( rep) 9999 !b | , | .. @p !b [ .. ] 'step .. @p !b @b ; .. ] !p ..
: start | *
: retry = $e then ( initial) 1024 | , | .. 959 for -1 . + | , | .. | .. dup trial - -while drop next retry ; | , | .. ] then pop drop ; | , | , = $18 reclaim exit | ,

{block 797}
( b is oscillator sequence generator) | , | , ] trial ( tries excitation at the period) [ n ( and) | , | .. ( returns a neg value if the osc node decided) | , | .. ( to try running with it.) n ( is in unext cycles) | .. ( with 6-bit fractional part.) | , | , ] start ( begins with a period much longer than is) | .. ( likely for the xtal in use, trying shorter) | , | .. ( periods in 1/64 unext or roughly 42ps mean) | , | .. ( i.e. interpolated period. returns to await) | , | .. ( when the osc node commits itself.) | ,
...
{block 822}
( characterize adcs) | , | , ( osc) 6 fh load | , ( adcs) 8 fh load | , ( wire) 10 fh load | , ( delivery) 12 fh load

{block 823}
( this application measures a-d values from) | , | .. ( nodes) # 709 # 713 # 717 ( and) # 617 ( using the fox osc) | , | .. ( on the ethernet board for time base. results) | .. ( acquired from) # 709 ( using snorkel/ganglia and) | , | .. ( are then logged and interpreted by polyforth) | ,

{block 824}
( - load descriptor) | , | , ( osc) 417 +node 417 /ram | , | .. $800 # $10800 $0 # $10000 2 /stack 0 /p | , ( wir) 317 +node 517 /ram up /b down /a 0 /p | , ( wir) 217 +node 517 /ram down /b up /a 0 /p | , ( adc) 117 +node 617 /ram 0 ldata right up | , | .. | .. 0 ldata right up 8 /stack 0 /p | , ( wir) # 117 ( +node) # 517 ( /ram up /b right /a) # 0 ( /p) | , ( wir) 116 +node 517 /ram right /b up /a 0 /p | , ( wir) 216 +node 517 /ram up /b down /a 0 /p | , ( wir) 316 +node 517 /ram down /b up /a 0 /p | , ( wir) 416 +node 517 /ram up /b down /a 0 /p | , ( wir) 516 +node 517 /ram down /b up /a 0 /p | , ( wir) 616 +node 517 /ram up /b right /a 0 /p | , ( adc) 617 +node 617 /ram 1 ldata down right | , | .. | .. 1 ldata down right 8 /stack 0 /p | , | , 2 fh load

{block 825}


{block 826}
( - more descriptor) | , | , ( adc) 717 +node 617 /ram 2 data right down | , | .. | .. 2 data right down 8 /stack 0 /p | , ( wir) 716 +node 517 /ram right /b left /a 0 /p | , ( wir) 715 +node 517 /ram left /b right /a 0 /p | , ( wir) 714 +node 517 /ram right /b left /a 0 /p | , ( adc) 713 +node 617 /ram 3 data right left | , | .. | .. 3 data right left 8 /stack 0 /p | , ( wir) 712 +node 517 /ram right /b left /a 0 /p | , ( wir) 711 +node 517 /ram left /b right /a 0 /p | , ( wir) 710 +node 517 /ram right /b left /a 0 /p | , ( adc) 709 +node 617 /ram 4 data down left | , | .. | .. 4 data down left 8 /stack 0 /p | , ( dlv) 609 +node 609 /ram 0 /p | ,

{block 827}


{block 828}
# 417 ( fox osc) 417 node 0 org reclaim | , | ,
: init = $0 io b! # 8 98 # 198 # 398 dup push | , | .. dup push dup push dup push dup push | , | .. dup push dup push dup push push | , | ... over over over over over over over over | , | .. up ( ...) | ,
: go = $b a! dup ! left a!
: spin = $e begin @ drop !b . @ drop !b . next | , | .. @ drop !b . @ drop !b . up go ; | , = $15 reclaim exit

{block 829}
( fox 819a-10) # 10 ( mhz oscillator input.) | , ( each pair of) # 50 ( ns edges is counted) | , | , ( each) # 50 ( ns edge sends a return instruction) | , ( thru up port to tx pin node) # 317 ( which waits) | , ( for the edge by calling or reading the port.) | , | , ( node) # 317 must ( wait faithfully for) every ( edge) | , ( for compatibility with soft oscillators.) | , | ,
: note ( node) # 317 ( starts us when it is ready to go by writing to the port.) | ,

{block 830}
( analog nodes) 617 node 0 org reclaim | , | ,
: init = $0 io b! ( vss) # $6000 ( vdd) $2000 !b | , | .. dup dup drop drop b! | ,
: run = $5 a! @b ! .. b! dup .. | , !b @b ! . push b! .. | , zif run ; ] then begin @b ! unext run ; | , | , = $d reclaim

{block 831}
( analog nodes need four parameters so we load) | , | .. ( them up on the stack. these are ...) | , | , | ... in/stim port | , | ... out/result port | , | ... data port for adc read | , | ... wds to pass after our answer | , | , ( stack is initialized with two copies of this.)
: init ( sets to have in port in t at start.)

{block 832}
( wire nodes) 517 node $38 org reclaim | ,
: !cnt ( n) = $38 @p drop !p ; | *
: cnt ( -n) = $39 -1 ; | , | , 0 org | ,
: init = $0 -1 | , | .. dup push dup push dup push dup push | , | .. dup push dup push dup push dup push | .
: wire @b dup ! !cnt wire ;
: wire begin begin @b ! unext unext wire ; | , | , = $b reclaim

{block 833}
( minimum latency wire node.) | , | , ( on entry b is incoming port, a outgoing.)

{block 834}
# 609 ( deliver data) 609 node 0 org reclaim ]
: run = $0 down a! io b! begin
: poll = $4 @b dup $200 and if ( snork) up b! @b @b
: pump = $a ( size) # 7999 9599 for @ !b unext run ; | , | .. ] then drop $2000 and ( if) until
: pass = $11 # 4 5 for @ unext ( then) poll ; | , = $15 reclaim

{block 835}
( the delivery node) # 609 ( waits for either an in-) | .. ( coming packet of) # 4 ( samples from) # 709 ( thru its) | .. ( down port or an incoming ganglion packet on) | , | .. ( its up port. when a ganglion packet is rcvd) | , | .. ( we discard the two words it gives us and) | , | .. ( wait for data from) # 709 ( for the agreed size) | , | .. ( of the transfer, must be multiple of) # 4 | , | , ( when data are received from) # 709 ( in between) | , | .. ( ganglion packets, we receive and discard) | , | .. ( four samples. this prevents loss of synch-) | , | .. ( ronization when polyforth is busy or slow.)
...
{block 848}
( framer template) empty | , | , compile streamer load framer load | , ( default) entire course | , | , ( setup application) | , | ... 600 +node $30000 /io | , | , ( make boot) frame | , | , stream oldflash ( newflash) | , | , ( feedback) .stream nores strlen leng ! | , ( flash) stream 0 swap 18burn

{block 849}
( framer template)

{block 850}
( framer host flash to target boot) | , empty compile streamer load framer load 2chip | , ( paths) 2 fh load brdg course | , ( speedup) 0 0 3 1608 /root | , ( bridge to target) | , 300 +node 1901 /ram $19 /p | , 400 +node 1907 /ram down /b up /a 0 /p | , 500 +node 1904 /ram 5 /a io /b down /p | , ( more to come) 705 +node exec /p | , ( make boot) frame | , | , fresh entire course | , ( target app) 4 fh load | , frame | , | , stream oldflash ( newflash) | , | , ( feedback) .stream nores strlen leng ! | , ( flash) stream 0 swap 18burn

{block 851}
( framer host flash to target boot) | , ( the first boot frame sets up the bridge) | , ( in both nodes) # 300 ( .) | , | , ( note that) 705 +node exec /p ( allows) | , ( a second boot frame to be loaded .) | , ( the default action of frame is to cause) | , ( the root node to jump to warm . this causes it to continue the boot stream instead .)

{block 852}
( framer bridge paths)
: brdg align create [ 705 700 to 600 300 to -1 , ]
: entire align create [ 705 405 to 404 400 to | , 10400 10700 to 10701 10717 to 10617 10601 to | , 10501 10517 to 10417 10401 to 10301 10317 to | , 10217 10200 to 10100 10117 to 10017 10000 to | , -1 ,

{block 853}
( framer bridge paths)
: brdg ( is a path to node) # 300 | , ( in order to setup a bridge and)
: entire ( is a path covering every node) | , ( in the target except the bridge in) # 300

{block 854}
( - app boot descriptors) | , 10600 +node 600 /ram 0 /p

{block 855}
( you could compile code for node) # 600 | , ( to be run in) # 10600 ( to prove that this works)
...
{block 882}
( pwr measures) 10100 node 0 org | ,
: one = $0 dup or begin . end
: two = $2 dup or begin - end
: thr = $4 dup or - dup begin - end
: fore = $6 dup or . dup begin - end
: fiv = $8 dup or . dup | , | ... begin begin . unext unext fiv ; | , | , = $b reclaim

{block 883}
( these routines were used 7/13/13 comparing) | , | .. ( pwr consumed in hardsim with real target) | , | .. ( chip on evb001 number) # 145

{block 884}
( 16-bit prn generator) 608 node 0 org ]
: random ( n-n) = $0 0 31421 a! 8 for +* . +* unext | , | .. drop drop a 6927 . + $7fff and ; = $b
...
: init = $0 io b! $20000 !b $3ffff | cr
dup dup dup dup dup dup dup dup dup | , .. . .. . .. . .. .
: loop = $b | cr
200 for = $d $3ffff for ( - dup and . . .) | , unext next | cr
= $10 # $10000 # $20000 $30000 !b | cr
200 for = $14 $3ffff for ( - dup and . . .) | , unext next | cr
= $17 $20000 !b | cr
loop ; = $1a

{block 895}
( things done gvb) | , ( - initialize io) | , ( - using bridge default stack get 6.5 ma steady - stack all 3ffff 5.75 steady) | , ( - change - and to dup and 4.155 steady)
...
{block 960}
( an012 ti sensortag) reclaim | , | , ( xtal osc) 20 fh load | , | .. ( hub timing) 22 fh load | , | .. | .. ( fake hub) 42 fh load | , | .. ( light sensor) 24 fh load | , | .. ( slow i2c bus) 28 fh 30 fh thru | , | .. ( node) # 705 40 fh load | , | , ( ganglia) # 44 ( fh load)

{block 961}
( this load block compiles the f18 code for all nodes in this application.)

{block 962}
( - load descriptors) | ,
: l, ( nn) dup +node 100 /mod 1 and 2* swap 1 and + 1714 + push 32 32 pop /part ( warm) $a9 /p ;
: ganglia nns for i # -1 [ 143 ] + n-nn l, next ; | , | , ( sea of mk1) ganglia | , ( cut current) 10705 +node 10705 /ram 0 /p | , ( slo-i2c) 10709 +node 10709 /ram 0 /p | , | .. ( bus) 10708 +node 10708 /ram 0 /p | , ( light) 10713 +node 10713 /ram 0 /p | , ( xtal) 10715 +node 10715 /ram $14 /p | , ( hub clk) 10717 +node 10717 /ram 0 /p | , | .. ( fake hub) 10617 +node 10617 /ram | , | ,

{block 963}
( this block describes entire chip's loading for polyforth boot environment.) | , | , ( suitable for use with ide loader, streamer,) | , ( and softsim.) | , | , ( all nodes not otherwise programmed are loaded with ganglia for ad hoc routing.) | , | , ( the sram cluster is invoked here for complete documentation although it has been loaded) | , ( earlier and is excluded from the path used by the main stream.) | , | , enable loading of clock nodes if you have ftdi or ether clock connected to 517. | , | , enable loading of ether cluster if you have | , nic hardware attached to right side of chip.

{block 964}
( -- code) | , | , exit ( buds) 205 +node 1705 /ram up /b 1 /p | , | ... 5 +node 1704 /ram down /b 1 /p | , | ... 206 +node 1702 /ram up /b right # 1 /p | , | ... 6 +node 1701 /ram down /b 1 /p | , | , ( bitsy) 105 +node 1703 /ram | , | ... $ffe0 $15555 ( 'cold) 1 3 /stack | , | ... left /b right /a $31 /p | , | , ( stack) 106 +node 1700 /ram | , | ... $ffff 0 0 3 /stack | , | ... right /b left /p
...
{block 970}
( an012 ide boot) bridge load loader load | , ( kill boots) 0 708 hook 0 -hook ]
: s1+ ( nm-mn') swap [ 1 ] + ; | , | , ( custom routes) 2 fh load | , ( select) myp1 'pths 1 + ! | , ( vm build tools) # 4 ( fh load) # 14 ( fh load) | , | , ( rest) 962 load | . 0 ship | . panel upd ?ram | ,
: z ( a-a) 2 + dup lit # $29 $31 call upd ;
: read ( n a) swap lit lit $34 call upd ;
: z ( k-k) $32 call upd ; | , ( test osc code) | , | .. 1 10617 hook | , ( put gyro to sleep) exit | , | .. 1 10708 hook $40 lit $3e lit $d0 lit $3c call
...
{block 972}
( - custom ide paths) | , | ,
: myp1 align create | , [ 708 608 to 607 600 to 500 400 to | , 10400 10600 to 10601 10617 to -1 , exit | , 10400 10414 to 10514 10714 to 10715 , -1 , | , 10400 10415 to 10515 10715 to -1 , exit | , 10400 10408 to 10508 10708 to -1 , | ,
...
: -osc ( kn-f) = $0 io b! for | , | .. = $2 $30000 !b dup .. 2/ dup for unext = $406 | , | .. $20000 !b .. over $1 and .. + for unext next | , | .. dup or !b dup 30000 for | , | .. | .. drop @b - -while next ; | , | ... | ... ] then dup or pop drop ;
: clang = $14 14600 50 for dup 5000 -osc while | , | .. drop 1 . + next ( clang) ; ] then ( pop drop ;) | ,
: prep = $1f $0 $20000 $800 $30800 $0 $20000 $800 $30800 | , | .. dup up a! drop
: run = $2c !b !b @ drop run ;
: try = $2e dup 5000 -osc over 1 . + ; | , = $34 reclaim

{block 981}
( 32.768 khz watch crystal from 715.17 to gnd) | , | , ( this code takes) # 27 ( ua at 2.1v using a seiko) | , ( vt200f-12.5pf-20ppm crystal,) # 42 ( cents unit qty from digi-key.) | , | ,
: -osc ( tries exciting the crystal with n cycles of period k returning nonzero if it didn't) | , ( come back high after last cycle.)
: clang ( searches for resonant frequency over a) | , ( reasonable range. initially we use) # 5000 ( cycles and may be able to shorten this. when we find resonance, falls thru into) prep ( which sets up registers and finally we camp in) run ( which is the low power, low duty cycle oscillator.)
...
{block 982}
# 717 ( hub timing) reclaim 10717 node 0 org | , host | *
: pu $d $15555 or ; | *
: -pu pu $800 or ; target | , | ,
: nit = $0 -pu lit pu lit over over | , | .. over over over over over up a! !b warm ;
: dly ( n) = $8 push begin dup ! !b unext ; | , | ,
: meas = $b 32768 dly meas ; | , = $e ( reclaim)

{block 983}
( this dac actively pulls sensor sda line up.)
: pu ( is io for wait-high,) -pu ( for wait-low.) | , | , ( this version does not supply an rtc as such.) | , ( it simply delays for the given number of clock edges thus consuming absolute minimal energy) | , ( with the 1-word loop) [ @ drop !b unext | , | , ( port calling sequence is...) | , | ... ( n) ] @p ! ! .. @p dly .. @p ! ; ] .. . .. | , ( first word passed gets arg and calls dly.) | , ( second word of nops waits for dly completion.) | , | ,
: meas ( runs the delay loop continually to) | , ( measure current when chip is waiting between) | , ( major cycles.)

{block 984}
# 713 ( light sensor) reclaim 10713 node 0 org | , host | *
: pu # $d 0 $15555 or ; target ]
: go = $0 pu lit !b warm ; | , = $3 reclaim

{block 985}
( this dac can pull our reset up but should not be used unless planning to run without evb pwr connected. powered host eats) # 37 ( ua and without host pwr it is about a ma.)
...
{block 988}
# 709 ( i2c timing) reclaim 10709 node 0 org | , host | *
: pu $d $15555 or ; target ]
: go = $0 pu lit !b warm ; | , = $3 reclaim

{block 989}
( this dac actively pulls sensor scl line up.)
...
: wnak c-d+ c+d+ ; | , | .. | *
: wack then c-d- c+d- ;
: w8 ( n-nak) = $13 2* 2* 2* 2* 2* 2* 2* 2* 2* | , | .. 7 for w1 next drop c-d* c+d* | *
: i1 @b $2 and ;
: rest ( a-ok) = $1e c-d+ | *
: strt = $1f ( @p) c+d+ c+d- w8 ( !p) ;
: stop = $22 c-d- c+d- c+d+ done ;
: r1 ( n-n) = $26 2* c-d* c+d* i1 or ;
: r8 ( -n) dup dup or 7 for r1 next 2/ ;
: r8+ ( -n) r8 wack ; | *
: r8- ( -n) r8 wnak ;
: @reg ( ia-snn) = $34 dup 1 or push | , | .. strt drop w8 | , | .. pop rest r8+ r8- stop ;
: !reg ( nia) = $3c strt drop w8 drop w8 stop ; | , = $40 reclaim exit | ,
: try = $35 ( a) io b! strt stop ;

{block 991}
port executable functions... ]
: done ( releases scl and sda to high impedance.)
: dly ( waits 1/2 bit time.)
: set ( stores io then waits 1/2 bit time.)
: c+d+ c+d- ( etc set bus state and delay.)
: strt ( sends start bit from hi-z, then address.)
: w1 ( xmits a bit clock cycle, leaving clk hi.)
: w8 ( xmits a byte, ret) # 2 ( if nak.)
...
{block 1000}
# 705 ( spi boot) reclaim 10705 node 0 org | ,
: go = $0 $5555 !b warm ; | , = $3 reclaim ( test pour voir si ca marche bien)

{block 1001}
( pin 705.17 has a 1k pull-up resistor causing) | , ( tens of ua vddi consumption in weak pd. this) | , ( code sets pin) # 17 ( hi-z to prevent that.)

{block 1002}
# 617 ( fake hub reclaim) 10617 node 0 org ]
: wait = $0 ( n) @p ! ! [ .. ] @p dly [ .. ] @p ! ; .. ] .
: pin = $4 ( n) @p !b !b ; .. ] @p !b | ,
: go = $6 down a! up b!
: run $30000 pin 327 wait $20000 pin 328 wait | , | .. run ; | , | .. ] $11111 $8000 wait $22222 $8000 wait $33333 ; | , = $1d reclaim
...
{block 1060}
( more tests) | , | ,
: rest ( a) lit $1f call ( upd) ;
: strt ( a) lit $20 call ( upd) ;
: stop $23 call ( upd) ; | , | ,
: try ( a) strt stop ;
...
{block 1152}
( install eforth via async bootstream) | , empty compile streamer load framer load | , ( async) | *
: frame [ $ae ] fram ; | , ( the stream) 2 fh load stream ( -an) | , | , serial load -canon # 0 ( fh orgn !) | , a-com sport ! a-bps bps ! !nam | , talk send panel | , | , ( test) exit ]
: s1+ ( nm-mn') swap [ 1 ] + ; ( sm) 454 load ]
: spa align create [ 708 108 to -1 , | , spa 'pths 1 + ! 1 108 hook panel
...
{block 1154}
( - generate eforth stream) | , | ,
: sram1 align create | , | .. [ 708 108 to 107 7 to 8 9 to -1 , sram1 course ( sram driver) sram 2 + load | , ( ide) 708 +node 1600 /ram $ae /p | , ( sram loader) 4 fh load frame | , ( read nucleus) 2 fh load @e4 | , ( header) $100ae !18 down !18 fbuf t@ 2 + !18 | , | .. drop !e4 ( exit test loaded sram) | , | , fresh ( residual) sram 4 + load s708 course | , | ... ( siobus and virtual machine) 1182 load | , ( ide) 708 +node 1600 /ram $ae /p frame

{block 1155}
( -)
: sram1 ( path just to load sram driver and data) | , | , ( read nucleus to get word count from address) # 0 | , | , ( construct a boot header by hand to send the) | , ( nucleus down to loader in node) # 108 ( ,) | , ( commented exit would allow reading and writing sram via the ide for testing) | , | , ( rest of nodes are loaded including) # 600 ( which) | , ( toggles it's pin for the scope , just a demo)

{block 1156}
( -- read eforth kernel) :var ft
: e4max ( -h) [ 16384 ] ; :var e4siz [0x133e]
: a-b ( a-b) 2* 2* ; | *
: b-a ( b-a) [ 3 ] + 2/ 2/ ;
: h-b 2* ; | *
: b-h [ 1 ] + 2/ ;
: allot h +! ; | *
: buffer pop ;
: fbuf align buffer [ 16384 allot ]
: +e4 [ 0 fnam b-a ] r/o fopen if [ ft ] ! ; | , | .. ] then drop abort ;
: -e4 [ ft ] @ [ 0 ] + if fclose 0 then [ ft ] ! ; | , | , macro | ,
: 2@ ( b-h) $8b66 3, ; | *
: swab ( h-h) $c486 2, ; forth ]
: t@ ( a-an) dup 2@ swab $ffff and ;
: @e4 +e4 if fbuf [ e4max ] h-b [ ft ] @ frd | , | .. b-h [ e4siz ] ! -e4 ; ] then abort ;
: !img ( nc) push [ fbuf ] begin t@ !18 [ 2 ] + | , | .. next drop ;
: !e4 [ fbuf ] t@ [ 2 ] + !img drop ; | , | , named ( ef/eforth.bin)

{block 1157}
( - nucleus in file ef/eforth.bin is read into a buffer on the host ,) | , | , !img ( uses) ] t@ ( to transform nucleus into correct byte order and bits per word while sending it through the serial interface via the ide's !18)
...
( sram driver) sram 2 + load | , ( flash to sram) 1174 load | , frame 1adjust ( resume point) | , | , fresh ( residual) sram 4 + load s705 course | , | ... ( siobus and virtual machine) 1182 load | , | ... ( flash-sram pipe) # 1184 ( load) | , | ... ( retain asynch boot) 708 +node $aa /p | , frame
...
{block 1190}
( burn 18-bit eforth bootstream into flash) | , ( 1st pass) empty compile streamer load | , ( framer) framer load ( the stream) 1180 load | , | , ( 2nd pass) empty compile streamer load | , ( framer) framer load ( the stream) 1180 load | , stream oldflash ( newflash) | , | , .stream ( display stream length) strlen leng ! | , ( write stream into flash) | , | ... ( source cf-word-addr) stream | , | ... ( dest flash-byte-addr) 0 | , | ... ( count 18-bit-words) swap ( flasher) 18burn | , | , serial load -canon options @ load | , ( burn nucleus) efbrn

{block 1191}


{block 1192}
( burn 16-bit eforth.bin file into new flash) | , empty compile streamer load | , # 1190 ( loaded !) .nucleus nores | , ( read eforth.bin) 1144 load | , fbuf source ! n-1 len ! | , ( display image length) len @ 8 18 */ leng ! | , | , stream $8000 swap 352 load

{block 1193}


{block 1194}
( burn 16-bit eforth.bin file into old flash) | , empty compile streamer load | , # 1190 ( loaded !) .nucleus nores | , ( read eforth.bin) 1144 load | , fbuf source ! n-1 len ! | , ( display image length) len @ 8 18 */ leng ! | , | , stream $8000 swap 258 load
...
{block 1304}
( eforth rom code) | , | , ( sdram addr) 9 node 1320 load | , | ... ( ctrl) 8 node 1322 load ( ram) | , | ... ( data) 7 node 1324 load | , | ... ( idle) 108 node 1326 load | , | ... ( user) 107 node 1328 load | , | , ( eforth stack) 106 node 1310 1312 thru | , | ... ( bitsy) 105 node 1306 1308 thru
...
{block 1332}
( spi flash writer 25wf080b) | , 705 node 0 org ]
: -cmd -++ half ;
: 2o ( dw-dw') = $2 obit 2* obit ;
: 2cmd ( dw-d) = $4 select 8obits
: cmd ( dw-d) select
: !8 ( dw-d) 8obits drop ;
: !word ( dw-d) 2* 2* 8obits !8 ;
: set ( -d) = $c io b! ( fast) 5 # 20 ;
: 8ibits ( d-db) = $f dup 7 for | , | .. rbit ibit - next $ff and ;
: @status = $16 set $1400 cmd 8ibits -cmd ;
: wait = $1b begin @status - 1 and until ;
: ers4k = $20 set ( wren+ers4k) # $1b60 $1880 # $1b5c 2cmd | , | .. 0 !8 = $25 0 = $427 8obits !8 wait ; | ,
: wrpg ( more page) = $29 push push set | , | .. ( wren+wrpg) $1808 2cmd pop !8 pop !8 0 !8 | , | .. 127 for @ !word next wait ;
: fet ( ah al-d) = $36 push push set ( read) $c00 cmd pop | .. !8 pop 8obits !8 ;
: 16ibits = $3d dup 15 push ibits ; = $40 | , 1612 bin

{block 1333}
( run by an adjacent node through a port.) | , ] $ 24 bit addresses ( are represented as a) # 256 ( byte page number which will be the middle byte of) | , ( the) # 24 ( bit address . all operations must start on) # 256 ( byte boundaries. assume) # 18 ( bit data) | , ( has been shifted and packed into) # 8 ( bit bytes) | , ( beforehand .) | , | ,
: wrpg ( writes one) # 256 ( byte page into flash) | , ( starting at address whose page byte is given. the low byte of the) # 24 ( address is zero .) | , ( the data are read from wherever) [ a ( points;) | , ( to feed a stream through a port,) a ( must) | , ( be focused to the port before starting.) | , ( read) # 16 ( bits at a time from) a ( .) | , | ,
: fet ( starts a read operation) | , | , ( wait reads) # 8 ( bits of status register) # 1 ( until) | , ( busy bit is zero)
...
{block 1340}
( - new flash burner helper) 706 node $20 org ]
: focus = $20 @p dup a! [ .. --l- ] .. ! ;
: writing = $23 ( h p) focus | , | .. left @p ! [ .. @p a! ] .. | , | .. ! @p ! [ .. @p @p ] .. push ! pop | , | .. ! @p ! ; .. $12029 , ( wrpg)
: reading = $2c ( l h) focus @p ! [ .. @p @p ] .. | , | .. ! ! .. @p ! ; .. $12036 , ]
: @word = $32 ( -n) @p ! [ .. $1203d , | , | .. ] @p ! @ ; .. ] !p ..
: stash = $36 focus @word !p ; = $39 0 , | , 1607 bin

{block 1341}
( - new flash bootstream burning helper)
: focus ( helper node gets) # 705 ( to pay attention)
: writing ( tell) # 705 ( to start writing flash)
: reading ( tell) # 705 ( to start reading flash)
: @word ( tell) # 705 ( to read) # 18 ( bit word from flash)
: stash ( read a word from flash and) | , | .. ( stash it in node) # 706 ( to be read by the ide)
...
{block 1400}
( f18 compiler) :var h [0x16] :var ip [0x15] :var slot [0x4] :var call? [0x6000] :var cal [0x102f9fca] :var com [0x6c] macro ]
...
: unext ( a) 4 i, drop ;
: n defer 8 f@ execute
: lit 8 i, ( ok in slot 3)
: , ( n) $3ffff and $15555 or p, ;
: for $1d i,
: begin here ;
: ahead ( -a) 2 adr ;
: end ( a) 2 -adr ;
